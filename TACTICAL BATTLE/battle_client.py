    ...
    while not victoria:
        ...
        if not miturno:
            ...
            if victoria:
                ...
                #Envío estado del equipo para la puntuacion
                estado_equipo = j1.miembros_vivos()
                estado_equipo_codificado = pickle.dumps(estado_equipo)
                s.sendall(estado_equipo_codificado)

                #Recibo ranking y lo imprimo
                ranking = s.recv(1024)
                ranking_str = ranking.decode()
                print("**** RANKING ****")
                print(ranking_str)
                break
        else:
            ...
            if not victoria:
                ...
                #Envío estado del equipo para la puntuacion
                estado_equipo = j1.miembros_vivos()
                estado_equipo_coificado = pickle.dumps(estado_equipo)
                s.sendall(estado_equipo_coificado)
